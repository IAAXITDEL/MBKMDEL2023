...
class FORequestdeviceController extends GetxController {
  ...
  Future<QuerySnapshot> getFODevices() async {
    ...
    if (user != null) {
      ...
      if (userSnapshot.docs.isNotEmpty) {
        ...
        QuerySnapshot snapshot = await _firestore.collection('pilot-device-1').where('user_uid', isEqualTo: userId).where('statusDevice',
            whereIn: ['in-use-pilot', 'in-use-fo', 'waiting-confirmation-1', 'need-confirmation-occ', 'waiting-handover-to-other-crew']).get();
    ...
    if (user != null) {
      ...
      if (userSnapshot.docs.isNotEmpty) {
        ...
        QuerySnapshot snapshot =
            await _firestore.collection('pilot-device-1').where('handover-to-crew', isEqualTo: userId).where('statusDevice', whereIn: [
  ...
  Future<bool> isDeviceInUse(String deviceUid2, String deviceUid3) async {
    ...
    QuerySnapshot snapshot1 = await _firestore.collection('pilot-device-1').where('device_uid2', isEqualTo: deviceUid2).where('statusDevice',
        whereIn: ['in-use-pilot', 'waiting-confirmation-1', 'need-confirmation-occ', 'waiting-confirmation-other-pilot']).get();

    QuerySnapshot snapshot = await _firestore.collection('pilot-device-1').where('device_uid', isEqualTo: deviceUid2).where('statusDevice',
        whereIn: ['in-use-pilot', 'waiting-confirmation-1', 'need-confirmation-occ', 'waiting-confirmation-other-pilot']).get();

    // Check if deviceUid is in 'device_uid2'
    QuerySnapshot snapshot2 = await _firestore.collection('pilot-device-1').where('device_uid3', isEqualTo: deviceUid3).where('statusDevice',
        whereIn: ['in-use-pilot', 'waiting-confirmation-1', 'need-confirmation-occ', 'waiting-confirmation-other-pilot']).get();
  ...
  void requestDevice(
    ...
    if (user != null) {
      ...
      QuerySnapshot deviceQuery = await _firestore.collection('Device').where('device_name', whereIn: [deviceName2, deviceName3]).get();
